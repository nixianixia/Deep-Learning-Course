'''
    浙江城市学院
    全连接，双隐藏层神经网络，这个代码由单隐藏层修改得到，主要为练习如何手动增加全连接的隐藏层
    minst 手写字符集 第个样本是一个 28 * 28 的灰度图，转为一维后，拥有 728 个输入特征
    tensorflow 2.0

    手动增加隐藏层过程
    确定新增加隐藏层的神经元数量 HX_NN
...
H1_NN = 64  # 隐藏层的神经元数量
W1 = tf.Variable(
    tf.random.normal([Input_Dim, H1_NN],
                     mean=0.0,
                     stddev=1.0,
                     dtype=tf.float32))
B1 = tf.Variable(tf.zeros([H1_NN], dtype=tf.float32))

# 定义第二隐藏层权重和偏置项变量        在此代码中新增的层
H2_NN = 32
W2 = tf.Variable(
    tf.random.normal([H1_NN, H2_NN], mean=0.0, stddev=1.0, dtype=tf.float32))
B2 = tf.Variable(tf.zeros([H2_NN]), dtype=tf.float32)

# 定义输出层权重和偏置项变量  #插入隐藏层后修改权重变量（W3)的形状
OutputDim = 10
W3 = tf.Variable(
    tf.random.normal([H2_NN, OutputDim],
                     mean=0.0,
                     stddev=1.0,
                     dtype=tf.float32))
B3 = tf.Variable(tf.zeros([OutputDim], dtype=tf.float32))

# 建立待优化变量列表 ，添加新层权重变量到待优化变量列表
W = [W1, W2, W3]
B = [B1, B2, B3]


# 定义模型前向计算         新加层后，需要增加新隐藏层前向计算
def model(x, w, b):
...
def loss(x, y, w, b):
    ...
    loss_ = tf.keras.losses.categorical_crossentropy(
        y_true=y, y_pred=pred)  # tensorflow 提供的交叉熵损失函数
...
def accuracy(x, y, w, b):
    ...
    correct_prediction = tf.equal(tf.argmax(pred, 1),
                                  tf.argmax(y, 1))  # 检查匹配 预测概率 与 标签的匹配
    return tf.reduce_mean(tf.cast(correct_prediction,
                                  tf.float32))  # 将布尔类型转为浮点数，计算平均值
